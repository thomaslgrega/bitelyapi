...
`gh` needs network access and an authenticated session. If the sandbox denies
either, say the tracker was unreachable rather than working from memory of it.
...
This repo owns the definitions the iOS app reads across the API boundary, so a
change to `CONTEXT.md` or a shared ADR lands here first.
...
No test reads `DATABASE_URL`; the suite runs without a database. Both commands
write to the module cache and build cache outside the repo (`go env GOMODCACHE
GOCACHE`), and a cold module cache also needs network, so they need an approval
mode that allows both.
