...
`GET /recipes?q=` answers a Name Query with Shared Recipes whose **name** the query reaches, ordered by how closely it reaches them. Two decisions the endpoint had to settle, and the reasons they went the way they did.
...
`category` composes with `q`, narrowing the search rather than widening it. Neither parameter is required on its own, but a request with neither is a 400 rather than the whole corpus.
